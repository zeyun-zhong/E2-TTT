...
def block_causal_lact_swiglu_with_cache(
    q, k, v,
    lr0, lr1, lr2,
    recurrent_state,
    chunk_size: int = 2048,
    ...
    # Read from cache
    (w0, w1, w2, buf_k, buf_v, buf_lr0, buf_lr1, buf_lr2, buf_momentum,
     dw0_momentum, dw1_momentum, dw2_momentum) = recurrent_state

    # cache new k, v, and others
    buf_k_new = torch.cat([buf_k, k], dim=1)
    buf_v_new = torch.cat([buf_v, v], dim=-1)
    buf_lr0_new = torch.cat([buf_lr0, lr0], dim=1)
    buf_lr1_new = torch.cat([buf_lr1, lr1], dim=1)
    buf_lr2_new = torch.cat([buf_lr2, lr2], dim=1)
    buf_momentum_new = torch.cat([buf_momentum, momentum], dim=1)
    ...
    if buf_k_new.shape[1] < chunk_size:
        ...
        out = torch.bmm(w1, gate * h).transpose(1, 2)
        return out, (w0, w1, w2, buf_k_new, buf_v_new, buf_lr0_new, buf_lr1_new, buf_lr2_new, buf_momentum_new, dw0_momentum, dw1_momentum, dw2_momentum)
    ...
    # [b, dh, dk] @ [b, dk, l] -> [b, dh, l]
    gate_before_act = torch.bmm(w0, buf_k_new.transpose(1, 2))
    hidden_before_mul = torch.bmm(w2, buf_k_new.transpose(1, 2))

    hidden = F.silu(gate_before_act, inplace=False) * hidden_before_mul

    # [b, dh, dv] @ [b, dv, l] -> [b, dh, l]
    dhidden = torch.bmm(w1.transpose(1, 2), buf_v_new)
    ...
    # it's better to cast the mat to bf16 before bmm.
    dw1 = torch.bmm(
        buf_v_new, (hidden.transpose(1, 2) * buf_lr1_new).type_as(buf_v_new)
    )  # [b, d, d]
    # [b, dh, l] @ [b, l, dk] -> [b, dh, dk]
    dw0 = torch.bmm(dgate_before_act, (buf_k_new * buf_lr0_new).type_as(dgate_before_act))
    dw2 = torch.bmm(dhidden_before_mul, (buf_k_new * buf_lr2_new).type_as(dhidden_before_mul))

    if momentum is not None:
        m_i = buf_momentum_new.mean(dim=1, keepdim=True)
    ...
    # Clear the cache
    bh, d = q.shape[:-1]
    buf_k = q.new_zeros(bh, 0, d)
    buf_v = q.new_zeros(bh, d, 0)
    buf_lr0 = q.new_zeros(bh, 0, lr0.shape[-1])
    buf_lr1 = q.new_zeros(bh, 0, lr1.shape[-1])
    buf_lr2 = q.new_zeros(bh, 0, lr2.shape[-1])
    buf_momentum = q.new_zeros(bh, 0, momentum.shape[-1])

    return out, (w0, w1, w2, buf_k, buf_v, buf_lr0, buf_lr1, buf_lr2, buf_momentum, dw0_momentum, dw1_momentum, dw2_momentum)
...
def block_causal_lact_swiglu(
    ...
    if s_index < e_index:
        ...
        buf_k = k[:, s_index:e_index, :]
        buf_v = v[:, :, s_index:e_index]
        buf_lr0 = lr0[:, s_index:e_index, :]
        buf_lr1 = lr1[:, s_index:e_index, :]
        buf_lr2 = lr2[:, s_index:e_index, :]
        buf_momentum = momentum[:, s_index:e_index, :]
    ...
    return output.transpose(1, 2), (w0, w1, w2, buf_k, buf_v, buf_lr0, buf_lr1, buf_lr2, buf_momentum, dw0_momentum, dw1_momentum, dw2_momentum)
