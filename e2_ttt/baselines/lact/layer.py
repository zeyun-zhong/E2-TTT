...
from e2_ttt.baselines.lact.ops import block_causal_lact_swiglu, l2_norm, block_causal_lact_swiglu_with_cache
...
class LaCTSWIGLULayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # [b * nh, s, d_ttt_head]
        # removed prenorm part, as it is deprecated by authors
        # post-norm version of ttt.   state = norm(state + f(state))

        if (not self.training) and (recurrent_state is not None):
            fw_x, recurrent_state = block_causal_lact_swiglu_with_cache(
            ...
            if recurrent_state is None:
                recurrent_state = (fw_w0, fw_w1, fw_w2)

            fw_x, recurrent_state = block_causal_lact_swiglu(
                fast_q, fast_k, fast_v,
                fw_lr1, fw_lr2, fw_lr3,
                recurrent_state,
                chunk_size=self.lact_chunk_size,
