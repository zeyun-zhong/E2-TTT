...
from e2_ttt.ops.e2_ttt_swiglu import (
    chunk_ttt_momentum,
...
class E2TTTSwiGLU(nn.Module):
    r"""
    E²-TTT with a SwiGLU fast weight ("E²-TTT_SwiGLU" in the paper).

    Same two-memory design as :class:`~e2_ttt.layers.E2TTTMLP`, but the fast
    weight is a SwiGLU MLP with three matrices ``(w0, w1, w2)`` instead of two,
    and the sliding-window attention branch can be switched off entirely
    (``use_window_attention=False``).

    Unlike the MLP variant this layer has no no-momentum kernel: the inner-loop
    momentum term is required.
    ...
    Args:
        ...
        use_memory_gate (bool, Optional):
            ...
            plain sum. Requires `use_window_attention`. Default: `False`.
        use_window_attention (bool, Optional):
            Keep the sliding-window attention branch. Default: `True`.
        ...
        use_ttt_momentum (bool, Optional):
            Enable the inner-loop momentum term. Required by this layer.
            Default: `False`.
        ...
        use_closed_form (bool, Optional):
            Use the paper's exact closed-form chunk transition. Setting it to
            `False` gives the "chunk-averaged" baseline. Default: `True`.
        use_post_norm (bool, Optional):
            Apply the post-chunk normalisation of the fast-weight readout.
            Default: `True`.
        use_fused_ttt_kernel (bool, Optional):
            Use the experimental fused Triton kernels instead of the reference
            PyTorch chunk update. Not used for any paper number. Default: `False`.
    """
    ...
    def __init__(
        ...
        use_fused_ttt_kernel: bool = False,
        **kwargs
    ):
        super().__init__()

        if not use_ttt_momentum:
            raise ValueError(
                "E2TTTSwiGLU requires use_ttt_momentum=True; there is no "
                "no-momentum SwiGLU kernel. Use E2TTTMLP for that ablation."
            )
        if use_memory_gate and not use_window_attention:
            raise ValueError("use_memory_gate requires use_window_attention=True.")
        ...
        self.w2 = nn.Parameter(torch.randn(num_heads, self.ttt_inter_dim, self.head_k_dim))
        ...
        self.use_fused_ttt_kernel = use_fused_ttt_kernel

    def reset_parameters(self):
        nn.init.normal_(self.w0, mean=0.0, std=1.0 / math.sqrt(self.head_k_dim))
        nn.init.normal_(self.w2, mean=0.0, std=1.0 / math.sqrt(self.head_k_dim))
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        if (not self.training) and (recurrent_state is not None):
            if self.use_fused_ttt_kernel:
                fn_ttt_cache = fused_chunk_ttt_momentum_with_cache
            else:
                fn_ttt_cache = chunk_ttt_momentum_with_cache
            ...
            if recurrent_state is None:
                recurrent_state = (self.w0, self.w1, self.w2)

            if self.use_fused_ttt_kernel:
                fn_ttt = fused_chunk_ttt_momentum
            else:
                fn_ttt = chunk_ttt_momentum
        ...
        if self.use_memory_gate:
            ...
            o = o_fw + o_kv if o_kv is not None else o_fw
